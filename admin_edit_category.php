...
if (isset($_REQUEST['editId']) and $_REQUEST['editId'] != "") {
	$row = $db->getAllRecords('category', '*', ' AND category_id="' . $_REQUEST['editId'] . '"');
}
...
if (isset($_REQUEST['submit']) and $_REQUEST['submit'] != "") {
	...
	$update = $db->update('category', $data, array('category_id' => $editId));
	if ($update) {
		header('location: admin_category.php?msg=rus');
		exit;
	} else {
		header('location: admin_category.php?msg=rnu');
		exit;
	}
}
?>
							...
							<form method="post">
								...
								<div class="form-group">
									...
									<input type="text" name="description" id="description" class="form-control"
										value="<?php echo $row[0]['description']; ?>" placeholder="Enter description"
										required>
								</div>
								<div class="form-group">
